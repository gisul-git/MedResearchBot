...
public partial class Admin_create_role : System.Web.UI.Page
{
            ...
            if (Page.IsValid)
            {
                ...
                if (btnSave.Text == "Update")
                {
                    if (CreateUser.CheckAccess(conMN, pageName, "Edit", Request.Cookies["med_aid"].Value))
                    {
                        ...
                        ca.Status = "Active";
                        int result = UserRoles.UpdateUserRoles(conMN, ca);
                        if (result > 0)
                        {
                            GetAllRoles();
                            GetRoles();
                ...
                {
                    if (CreateUser.CheckAccess(conMN, pageName, "Add", Request.Cookies["med_aid"].Value))
                    {
                        ca.RoleName = txtName.Text;
                        ca.UpdatedOn = TimeStamps.UTCTime();
                        ca.Status = "Active";
                        int result = UserRoles.InsertUserRoles(conMN, ca);
                        if (result > 0)
                        {
                            txtName.Text = "";
                            GetAllRoles();
    ...
    public void GetAllRoles()
    {
        ...
        {
            strUserRoless = "";
            List<UserRoles> cas = UserRoles.GetAllUserRoles(conMN).OrderByDescending(s => Convert.ToDateTime(s.UpdatedOn)).ToList();
            int i = 0;
...
<td>" + ca.UpdatedOn.ToString("dd-MMM-yyyy") + @"</td>  
    ...
    public static string Delete(string id)
    {
        ...
        {
            SqlConnection conMN = new SqlConnection(ConfigurationManager.ConnectionStrings["conMN"].ConnectionString);
            if (CreateUser.CheckAccess(conMN, "create-role.aspx", "Delete", HttpContext.Current.Request.Cookies["med_aid"].Value))
            {
                UserRoles cat = new UserRoles();
                cat.Id = Convert.ToInt32(id);
                cat.UpdatedOn = TimeStamps.UTCTime();
                int exec = UserRoles.DeleteUserRoles(conMN, cat);
                if (exec > 0)
                {
                    x = "Success";
                }
