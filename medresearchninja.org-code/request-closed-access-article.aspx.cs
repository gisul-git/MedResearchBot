...
public partial class request_closed_access_article : System.Web.UI.Page
{
    ...
    protected void btnPortfolio_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            if (Page.IsValid)
            {
                RequestClosedAccessArticle req = new RequestClosedAccessArticle();
                ...
                int result = RequestClosedAccessArticle.InsertRequestClosedAccessArticle(conMN, req);
                if (result > 0)
                {
                    ClearFormFields();
