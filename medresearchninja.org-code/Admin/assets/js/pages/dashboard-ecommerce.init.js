...
$.ajax({
    ...
    success: function (data) {
        ...
        $(".totalSales").html(data.d.TotalOrder.toLocaleString());
        $(".confirmedSale").attr("data-target", data.d.TotalPaid);
        $(".confirmedSale").html(data.d.TotalPaid.toLocaleString());
        $(".initiatedSale").attr("data-target", data.d.TotalInitiated);
        $(".initiatedSale").html(data.d.TotalInitiated.toLocaleString());
...
var options,
    ...
    vectorMapWorldMarkersColors =
        (((options2 = {
            series: PayStatus,
            labels: ["Initiated", "In-Process", "Completed"],
            chart: { height: 333, type: "donut" },
            legend: { position: "bottom" },
            stroke: { show: !1 },
            dataLabels: { dropShadow: { enabled: !1 } },
            colors: ['#ffbe0b', '#299cdb', '#45CB85'],
        }),
...
$(document).ready(function () {
    ...
    $(".filterSts").click(function () {
        $("#store-visits-source").empty();
        $(".filterSts").removeClass("selected");
        $(this).addClass("selected");

        var fValue = $(this).attr("data-val");
        $.ajax({
            type: 'POST',
            url: "dashboard.aspx/FilterDashBoardChartStatus",
            data: "{fValue:'" + fValue + "'}",
            contentType: 'application/json; charset=utf-8',
            dataType: "json",
            async: false,
